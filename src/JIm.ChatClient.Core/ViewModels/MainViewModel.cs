...
using JIm.ChatClient.Core.Config;
using System.Windows.Input;
using JIm.ChatClient.Core.Views;
using JIm.ChatClient.Core.Converters;
...
namespace JIm.ChatClient.Core.ViewModels
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        public MainViewModel()
        {
            ...
            // 从数据库加载模型配置
            LoadModelsFromDatabaseAsync().ConfigureAwait(false);
        ...
        private async Task LoadModelsFromDatabaseAsync()
        {
            try
            {
                // 首先尝试将注册表中的模型迁移到数据库（仅在首次运行时执行）
                await _databaseService.MigrateModelsFromRegistryAsync();
                ...
                foreach (var model in models)
                {
                    ...
                    if (model.IsDefault)
                    {
                        SelectedModel = AvailableModels.Last();
                    }
                }
            ...
            {
                Console.WriteLine($"从数据库加载模型配置失败: {ex.Message}");
                
                // 如果从数据库加载失败，回退到从注册表加载
